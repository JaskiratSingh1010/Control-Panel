...
(function () {
  ...
  function wireDate(input) {
    ...
    /* Draw our calendar icon only when the field has not got one already. Pages like
       Customer Aging's "As of" box put their own icon in the wrapper and size the
       input to its text, so a second icon sat right on top of the date. */
    var box = input.parentElement;
    if (!box || !box.querySelector('.ic, .material-icons, .material-symbols-outlined, svg, img')) {
      input.classList.add('bkdp-ico');
    }
