...
function setRealiseLine(el,litres,perLitre,what){
  if(!el)return;
  var rate=(perLitre&&isFinite(perLitre))?Number(perLitre):0;
  if(rate>0){
    el.innerHTML=fNp(rate,2)+'<span class="u">/LTR</span><span class="sep">&middot;</span>'
                +fRsShort((litres||0)*rate);
    ...
    el.innerHTML='<span class="u">no revenue data</span>';
