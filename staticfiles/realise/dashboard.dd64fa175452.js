...
/* ── Premium / Commodity split shown on every KPI card ──────────────────────
   Two small legend rows per card: litres under the headline figure, and realise
   under the foot figure. Both halves come from the SAME rows the card total was
   built from, so they always add up to what is shown above them.
     segSplit()        for sales / open-order rows (value ÷ litres = realise)
     segSplitTargets() for target nodes, which carry their own realise per node,
                       so it is weighted by litres exactly like the cards do.
   Shape: {p:{ltr,rate}, c:{ltr,rate}}  - p = Premium, c = Commodity. */
function segSplit(rows,ltrKey,valKey){
  var p={l:0,v:0}, c={l:0,v:0};
  for(var i=0;i<(rows||[]).length;i++){
    var r=rows[i], k=String(r.u_type||'').trim().toUpperCase();
    var b=(k==='PREMIUM')?p:((k==='COMMODITY')?c:null);
    if(!b)continue;
    b.l+=Number(r[ltrKey])||0; b.v+=Number(r[valKey])||0;
  }
  return {p:{ltr:p.l,rate:p.l>0?p.v/p.l:0}, c:{ltr:c.l,rate:c.l>0?c.v/c.l:0}};
}
function segSplitTargets(nodes){
  var p={l:0,w:0}, c={l:0,w:0};
  for(var i=0;i<(nodes||[]).length;i++){
    var n=nodes[i], tl=Number(n.target_ltrs)||0; if(!tl)continue;
    // A blank segment counts as Premium here, the same as the tables do.
    var b=(String(n.segment||'').trim().toUpperCase()==='COMMODITY')?c:p;
    b.l+=tl; b.w+=(Number(n.target_realise)||0)*tl;
  }
  return {p:{ltr:p.l,rate:p.l>0?p.w/p.l:0}, c:{ltr:c.l,rate:c.l>0?c.w/c.l:0}};
}
// Bal has no rows of its own: it is what is left of each half of the target.
function segSplitBal(tgt,done,oih){
  function left(a,b,c){ return (a||0)-(b||0)-(c||0); }
  if(!tgt)return null;
  return {p:{ltr:left(tgt.p.ltr,done&&done.p.ltr,oih&&oih.p.ltr),rate:0},
          c:{ltr:left(tgt.c.ltr,done&&done.c.ltr,oih&&oih.c.ltr),rate:0}};
}
/* Paint one legend row. kind 'ltr' shows whole litres, 'rate' shows ₹ per litre. */
function setSegSplit(el,split,kind){
  if(!el)return;
  if(!split){ el.innerHTML=''; return; }
  function one(cls,label,v){
    var shown=(v||0)!==0 ? (kind==='rate'?fNp(v,2):fN(Math.round(v))) : '&mdash;';
    return '<span class="k-seg '+cls+'"><i></i><b>'+label+'</b><span>'+shown+'</span></span>';
  }
  el.innerHTML=one('k-prem','Premium',kind==='rate'?split.p.rate:split.p.ltr)
              +one('k-comm','Commodity',kind==='rate'?split.c.rate:split.c.ltr);
}
function setRealiseLine(el,litres,perLitre,what,emptyText){
  if(!el)return;
  var rate=(perLitre&&isFinite(perLitre))?Number(perLitre):0;
  if(rate>0){
    // The rate is the headline of the foot strip (as in the E-Commerce cards, where the
    // "/Ltr" figure is set large); the money it comes to sits quietly on the right.
    el.innerHTML='<span class="k-rate">'+fNp(rate,2)+'<span class="u">/LTR</span></span>'
                +'<span class="k-money">'+fRsShort((litres||0)*rate)+'</span>';
    el.title='Realise of the '+what+' litres — the ₹ per litre, and what those litres come to';
  }else if((litres||0)>0){
...
function setSlideTwoKpis(targetLtr,doneLtr,targetRealise,currentRealise,oihLtr,balLtr,oihRealise,splits){
  ...
  /* Premium / Commodity legends: litres under the headline figure, realise under the
     foot figure. Bal has no realise of its own, so it only gets the litres row. */
  var sp=splits||{};
  setSegSplit(document.getElementById('sc2KpiTargetSplit'), sp.target, 'ltr');
  setSegSplit(document.getElementById('sc2KpiDoneSplit'),   sp.done,   'ltr');
  setSegSplit(document.getElementById('sc2KpiOihSplit'),    sp.oih,    'ltr');
  setSegSplit(document.getElementById('sc2KpiBalSplit'),    sp.bal,    'ltr');
  setSegSplit(document.getElementById('sc2KpiTargetRealiseSplit'), sp.target, 'rate');
  setSegSplit(document.getElementById('sc2KpiDoneRealiseSplit'),   sp.done,   'rate');
  setSegSplit(document.getElementById('sc2KpiOihRealiseSplit'),    sp.oih,    'rate');
  sc2RenderTopKpis();
}
...
function renderSlideTwoCommodity(){
  ...
  // Everything on this view is Commodity by definition, so the Premium half is empty.
  function comHalf(l,r){ return {p:{ltr:0,rate:0},c:{ltr:l||0,rate:r||0}}; }
  var cT=comHalf(tot.target,tot.targetRealise),
      cD=comHalf(tot.done,tot.done>0?tot.lineTotal/tot.done:0),
      cO=comHalf(tot.oih,(tot.oih||0)>0?((tot.oihLineTotal||0)/tot.oih):0);
  setSlideTwoKpis(tot.target,tot.done,tot.targetRealise,tot.done>0?tot.lineTotal/tot.done:0,tot.oih,
                ...
                {target:cT,done:cD,oih:cO,bal:segSplitBal(cT,cD,cO)});
...
async function renderSlideTwo(){
  ...
  if(mode!=='main_group'){
    ...
    var dspT=segSplitTargets(dTargetNodes), dspD=segSplit(rows,'liter','line_total'),
        dspO=segSplit(dOihRows,'open_qty','open_value');
    setSlideTwoKpis(dKpiTarget,dt.done,dt.targetRealise,sc2Realise(rows),dt.oih,dBal,
                    (dt.oih||0)>0?((dt.oihLineTotal||0)/dt.oih):0,
                    {target:dspT,done:dspD,oih:dspO,bal:segSplitBal(dspT,dspD,dspO)});
  ...
  var spT=segSplitTargets(targetNodes), spD=segSplit(rows,'liter','line_total'),
      spO=segSplit(oihRows,'open_qty','open_value');
  setSlideTwoKpis(kpiTarget,agg.done,kpiTargetRealise,sc2Realise(rows),agg.oih,undefined,
                  ...
                  {target:spT,done:spD,oih:spO,bal:segSplitBal(spT,spD,spO)});
