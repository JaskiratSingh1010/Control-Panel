...
function setSlideTwoKpis(targetLtr,doneLtr,targetRealise,currentRealise,oihLtr,balLtr){
  ...
  /* Realise of the Done litres, under the Done Ltr figure, in two ways:
       rate  = ₹ per litre  — sc2Realise(): total line_total ÷ total litres. This is
                              the figure the old "Actual Realise" card used to show
                              on its own; that card was dropped once this line existed.
       money = rate × Done litres = the revenue those litres earned.
     Both are worked out from the values already passed in, so not one caller of
     setSlideTwoKpis had to change. */
  var elDoneRealise=document.getElementById('sc2KpiDoneRealise');
  if(elDoneRealise){
    var perL=(currentRealise&&isFinite(currentRealise))?Number(currentRealise):0;
    if(perL>0){
      // No ₹ on the per-litre rate (the "/LTR" already says what it is); the money
      // figure beside it keeps its ₹.
      elDoneRealise.innerHTML=fNp(perL,2)+'<span class="u">/LTR</span><span class="sep">&middot;</span>'
                              +fRsShort((doneLtr||0)*perL);
      elDoneRealise.title='Realise of the Done litres — the ₹ earned per litre, and the '
                         +'money those litres brought in';
    }else if((doneLtr||0)>0){
      // Litres are there but no revenue came back with them, so ₹/litre cannot be worked out.
      // Say that rather than showing a bare dash, which looks like the figure failed to load.
      elDoneRealise.innerHTML='<span class="u">no revenue data</span>';
      elDoneRealise.title='These Done litres carry no line total, so a ₹ per litre realise cannot be calculated';
    }else{
      elDoneRealise.innerHTML='&mdash;';
      elDoneRealise.title='';
    }
  }
  sc2RenderTopKpis();
